...
function CinematicState:update(dt)
    ...
    if player.x >= MAP_WIDTH - 200 then
        ...
        if self.dayNumber == 5 then
            ...
            stateMachine:change('play',
                { player = player, dayNumber = self.dayNumber + 1, isANewDay = true, player2 = player2,
    ...
    if player2 ~= nil then
        ...
        math.floor(math.max(self.camera.x,
            math.floor((player.x + player.width / 2 - VIRTUAL_WIDTH / 2 + player2.x + player2.width / 2 - VIRTUAL_WIDTH / 2) /
            2))), math.floor(MAP_WIDTH - VIRTUAL_WIDTH)))
    else
        self.camera.x = math.floor(math.min(
        math.floor(math.max(self.camera.x, player.x + player.width / 2 - VIRTUAL_WIDTH / 2)),
