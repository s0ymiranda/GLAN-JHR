...
function Dialog:set(centerX, bottomY, message)
    ...
    self.height = (#self.textChunks - 1) * (self.textHeight + self.leading) + self.textHeight + 2 * (self.outlineWidth + self.margin)
...
function Dialog:render()
    ...
    love.graphics.rectangle('fill', self.x + self.outlineWidth, self.y + self.outlineWidth, self.width - 2 * self.outlineWidth, self.height - 2 * self.outlineWidth, 3)
    love.graphics.setColor(love.math.colorFromBytes(0, 0, 0, 255))

    -- Text
    love.graphics.setFont(self.font)
    for i = 1, #self.textChunks do
        love.graphics.print(self.textChunks[i], self.x + self.outlineWidth + self.margin, self.y + self.outlineWidth + self.margin + (self.textHeight + self.leading) * (i - 1))
    end
    love.graphics.setColor(love.math.colorFromBytes(255, 255, 255, 255))
end
